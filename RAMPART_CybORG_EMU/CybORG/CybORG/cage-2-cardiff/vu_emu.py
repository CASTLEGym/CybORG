...
blue_action_space = ['DecoyApache', 'DecoySSHD', 'DecoyVsftpd', 'Restore', 'DecoyFemitter', 'Remove', 'DecoyTomcat',
                     'DecoyHarakaSMPT', 'Analyse']
blue_decoys = ['DecoyApache', 'DecoySSHD', 'DecoyVsftpd', 'DecoyTomcat', 'DecoyFemitter',
               'DecoyHarakaSMPT']  # windows specific decoys are faked.
red_action_space = ['PrivilegeEscalate', 'ExploitRemoteService', 'DiscoverRemoteSystems', 'DiscoverNetworkServices']
vms = ["User0", "User1", "User2", "User3", "User4", "Enterprise0", "Enterprise1", "Enterprise2", "Op_Host0", "Op_Host1",
       "Op_Host2", "Op_Server0", "Defender"]
red_info = {"User0"}
blue_info = {}
...
class vu_emu():
                ...
                if action_name in blue_action_space:
                    ...
                    if modify_blue_red == True:
                        ...
                        if self.last_red_action in red_actions_that_affect_blue:
                            # print('In modify')
                            outcome = self.modify_blue_by_red(outcome, self.old_outcome_red, self.last_red_action,
                                                              self.last_red_action_param)
    ...
    def execute_action_client(self, action_name, action_param, running_from=None):
        ...
        if action_name == 'DiscoverRemoteSystems':
            ...
            filtered_list = [ip for ip in ip_list if
                             not ip.endswith('.100') and not ip.endswith('.120') and not ip.endswith('.200')]
            ...
            if action_param in self.exploited_hosts:
                ...
                action = ExploitAction(credentials_file, red_intial_foothold, action_param, 'ubuntu', 'ubuntu', port,
                                       server_port)
                observation = action.execute(None)
                success = enum_to_boolean(str(observation.success))
            ...
            if action_param in self.connection_key:
                ...
                action = PrivilegeEscalateAction(credentials_file, red_intial_foothold,
                                                 self.connection_key[action_param], action_param, 'ubuntu', 'ubuntu',
                                                 client_port)
                observation = action.execute(None)
                success = enum_to_boolean(str(observation.success))
        ...
        ### Blue Actions
            ...
            restore_action = RestoreAction(
                hostname=cage2os_instance.fetch_alt_name(ip2host.fetch_alt_name(action_param)),
                auth_url=self.url,
