...
class GameState:
    def __init__(self, episode, step, compromised_hosts=None, exploited_hosts=None, escalated_hosts=None, discovered_subnets=None, discovered_systems=None):
        self.episode = episode
        self.step = step
        # depends on scenarios
        self.compromised_hosts = compromised_hosts if compromised_hosts is not None else set(['User0'])
        self.exploited_hosts = exploited_hosts if exploited_hosts is not None else set()
        self.escalated_hosts = escalated_hosts if escalated_hosts is not None else set(['User0'])
        self.discovered_subnets = discovered_subnets if discovered_subnets is not None else set()
        self.discovered_systems = discovered_systems if discovered_systems is not None else set()
...
class GameHistory:
    ...
    def get_state(self, episode, step):
        # Logic to retrieve a specific GameState instance
        for state in self.states:
            if state.episode == episode and state.step == step:
                return state
        return None
...
class IPHostMapper:
    ...
    def create_ip_host_maps(self, cyborg):
        # Logic to create IP and host mappings
